...
Tags: [[physics]] [[computer science]] [[quantum computers]]

# Quantum Computing Basics

## Qubits

We define them as column vectors $$\vert 0 \rangle =\begin {pmatrix}1\\0\end{pmatrix} \textrm{  and  }\vert 1 \rangle =\begin {pmatrix}0\\1\end{pmatrix}$$
#### Superposition
...
A qubit is in superposition if it is both $$\vert 0 \rangle \textrm{  and  } \vert 1 \rangle$$ at the same time.
...
The probabilities of $\alpha$ and $\beta$ must always add up to 1 so that we have a valid state of a qubit:  $$\vert \alpha \vert² + \vert \beta \vert² = 1$$ 
#### Dirac Notation
...
![[Pasted image 20241215162512.png]]
...
We can represent these three gates as matrices.  $$ X = \begin {pmatrix}0&&1\\1 && 0\end{pmatrix} \quad Y = \begin {pmatrix} 0 && -i\\ i && 0\end{pmatrix} \quad Z = \begin {pmatrix}1&&0\\0 && -1\end{pmatrix} $$
To apply the gate to the qubit we can mathematically multiply the matrix that represents the gate with the column vector of the qubit. $$X\vert\psi\rangle = \begin {pmatrix}0&&1\\1 && 0\end{pmatrix} \begin{pmatrix}\alpha\\\beta\end{pmatrix} = \begin {pmatrix}\beta\\\alpha\end{pmatrix}$$
...
Probability of measuring $$1=\vert e^{i\varphi}\beta\vert0\rangle\vert ^2\Rightarrow 1=\vert e^{i\varphi} \vert^2\ \  \vert\beta\vert0\rangle\vert ^2$$  
...
We can represent two qubits with a Tensor Product. $$\vert0\rangle \otimes \vert0\rangle = \vert00\rangle $$
...
![[Pasted image 20241215192054.png]]
...
#### CNOT
![[Pasted image 20241215195953.png]]
...
$$A = \sqrt{\frac{1}{x_1^2 + x_2^2 + ....+ x_n^2}}$$
...
Lets say that Alice wants to send Bob two classical bits. Alice and Bob both have one qubit. Depending on what two bits Alice wants to send she will preform some operations on her qubit.
 The first qubit is Alice's and the second one is Bobs. $$\vert\psi\rangle=\frac{1}{\sqrt{2}}(\ \vert00\rangle+\vert11\rangle\ )$$
...
![[Pasted image 20241216151113.png]]
